// wM-Buster ADV — DES-CBC decryption (FIPS 46-3) for the deprecated wM-Bus
// TPL security modes 2 (IV = 0) and 3 (IV = id + mfct + date).
// GPL-3.0
#include "wmbus/aes.h"
...
// Permutation tables, 1-indexed bit positions counted from the MSB.
...
static const uint8_t EXP[48] = { 32, 1,  2,  3,  4,  5,  4,  5,  6,  7,  8,  9,  8,  9,  10, 11,
...
static const uint8_t PERM[32] = { 16, 7, 20, 21, 29, 12, 28, 17, 1,  15, 23, 26, 5,  18, 31, 10,
...
static uint64_t permute(uint64_t src, int src_bits, const uint8_t* table, int dst_bits) {
    uint64_t dst = 0;
    for (int i = 0; i < dst_bits; ++i) dst = (dst << 1) | ((src >> (src_bits - table[i])) & 1);
    return dst;
}

static uint32_t rot28(uint32_t v, int n) { return ((v << n) | (v >> (28 - n))) & 0x0FFFFFFF; }

static void key_schedule(const uint8_t key[8], uint64_t sub[16]) {
    uint64_t k = 0;
    for (int i = 0; i < 8; ++i) k = (k << 8) | key[i];
    uint64_t kp = permute(k, 64, PC1, 56);
    uint32_t c = (uint32_t)(kp >> 28) & 0x0FFFFFFF, d = (uint32_t)kp & 0x0FFFFFFF;
    for (int r = 0; r < 16; ++r) {
        c = rot28(c, ROT[r]);
        d = rot28(d, ROT[r]);
...
static uint32_t feistel(uint32_t r, uint64_t subkey) {
    uint64_t e = permute(r, 32, EXP, 48) ^ subkey;
    uint32_t s = 0;
    for (int i = 0; i < 8; ++i) {
    ...
    return (uint32_t)permute(s, 32, PERM, 32);
}

static void block_decrypt(const uint8_t in[8], uint8_t out[8], const uint64_t sub[16]) {
    uint64_t b = 0;
    for (int i = 0; i < 8; ++i) b = (b << 8) | in[i];
    b = permute(b, 64, IP, 64);
    uint32_t l = (uint32_t)(b >> 32), r = (uint32_t)b;
    for (int k = 15; k >= 0; --k) {
        uint32_t nr = l ^ feistel(r, sub[k]);
        l = r;
        r = nr;
    }
    uint64_t res = permute(((uint64_t)r << 32) | l, 64, FP, 64);
    for (int i = 7; i >= 0; --i) {
        out[i] = (uint8_t)res;
        res >>= 8;
    }
...
bool des_cbc_decrypt(const uint8_t key[8], const uint8_t iv[8], const uint8_t* in, uint8_t* out, size_t len) {
    ...
    uint64_t sub[16];
    key_schedule(key, sub);
    uint8_t prev[8], cur[8], blk[8];
    memcpy(prev, iv, 8);
    for (size_t o = 0; o < len; o += 8) {
        memcpy(cur, in + o, 8);
        block_decrypt(cur, blk, sub);
        for (int i = 0; i < 8; ++i) out[o + i] = blk[i] ^ prev[i];
        memcpy(prev, cur, 8);
    }
    return true;
}
